...
export default function Companions() {
  ...
  const handleSendRequest = async (receiverCompanionId) => {
    if (!isLoggedIn) {
      showToast('Please login to connect', 'warning');
      return;
    }
    try {
      // Find the user's companion profile for the same destination
      const destName = companions.find(c => c.id === receiverCompanionId)?.destinationName;
      const myProfile = companions.find(c => c.userId === user?.userId && c.destinationName === destName);
      
      if (!myProfile) {
        showToast('You need a profile for this destination first!', 'error');
        return;
      }

      await api.post('/matches/request', {
        myCompanionId: myProfile.id,
        receiverCompanionId,
        message: 'Hi, I would love to connect and explore together!'
      });
