...
export default function Home() {
  ...
  const [heroType, setHeroType] = useState('SOLO');
  ...
  const handleSearch = () => {
    if (heroDate || heroType !== 'SOLO') {
      const params = new URLSearchParams();
      if (heroSearch) params.append('destName', heroSearch);
      if (heroDate) params.append('date', heroDate);
      if (heroType) params.append('type', heroType);
            ...
            <div className="hero-card">
              ...
              <div className="search-group">
                ...
                <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }} onClick={handleSearch}>
                  <i className="fas fa-search"></i> {heroDate || heroType !== 'SOLO' ? 'Plan Trip' : 'Search Destinations'}
                </button>
              </div>
            </div>
          </div>
